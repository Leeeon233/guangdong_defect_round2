...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if self.style == 'pytorch':
            ...
            self.conv2_stride = 1
        self.with_blur = with_blur if stride != 1 else False
        ...
        if not self.with_dcn or fallback_on_stride:
            self.conv2 = build_conv_layer(
                ...
                planes,
                kernel_size=3,
                stride=self.conv2_stride if not with_blur else 1,
                padding=dilation,
                dilation=dilation,
                bias=False)
        else:
            ...
            self.conv2 = conv_op(
                ...
                bias=False)
        if with_blur:
            self.dc = Downsample(stride=self.conv2_stride, channels=planes, filt_size=5)
        self.add_module(self.norm2_name, norm2)
...
class SiameseResNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            res_layer = make_res_layer(
                ...
                with_cp=with_cp,
                with_se=with_se,
                with_blur=False,   # TODO change version
                conv_cfg=conv_cfg,
                norm_cfg=norm_cfg,
                dcn=dcn,
                gcb=gcb,
    ...
    def _make_stem_layer_blur(self):
        ...
        self.add_module(self.norm1_name, norm1)
        self.relu = nn.ReLU(inplace=True)
        # self.dc1 = Downsample(stride=2, channels=64, filt_size=5)
    ...
    def forward(self, _x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if not self.with_abs:
                ...
                x2 = res_layer(x2)
                if i in self.out_indices:
                    outs.append(torch.cat([x-x2, x], dim=1))
        return tuple(outs)
