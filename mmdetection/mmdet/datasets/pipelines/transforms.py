...
class ResizeImages(object):
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            template_img, _ = mmcv.imrescale(
                results['img'][1], results['scale'], return_scale=True)
        else:
            img, w_scale, h_scale = mmcv.imresize(
                results['img'][0], results['scale'], return_scale=True)
            template_img, _, _ = mmcv.imresize(
                results['img'][1], results['scale'], return_scale=True)
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
        results['img'] = [img, template_img]
...
class PadImages(object):
    ...
    def _pad_img(self, results):
        if self.size is not None:
            padded_img = mmcv.impad(results['img'][0], self.size)
            template_padded_img = mmcv.impad(results['img'][1], self.size)
        elif self.size_divisor is not None:
            padded_img = mmcv.impad_to_multiple(
        ...
        results['img'] = [padded_img, template_padded_img]
